"""
Russell Reynolds Plugin — Executive search firm.
...
class RussellReynoldsPlugin(SourcePlugin):
    """Russell Reynolds Associates — executive search and assessment."""

    SPEC = ALL_PLUGINS["russell_reynolds"]

    @retry(stop=stop_after_attempt(3), wait=wait_exponential(min=1, max=15))
    async def discover(self, query: ContactQuery) -> list[SourceResult]:
        results: list[SourceResult] = []
        url = "https://www.russellreynolds.com/consultants"
            ...
            for profile in profiles[:10]:
                ...
                if name_match:
                    results.append(self._make_source_result(
                        ...
                        extracted_company="Russell Reynolds Associates",
