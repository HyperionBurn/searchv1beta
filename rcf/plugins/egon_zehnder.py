"""
Egon Zehnder Plugin — Executive search firm.
...
class EgonZehnderPlugin(SourcePlugin):
    """Egon Zehnder — global executive search and leadership consulting."""

    SPEC = ALL_PLUGINS["egon_zehnder"]

    @retry(stop=stop_after_attempt(3), wait=wait_exponential(min=1, max=15))
    async def discover(self, query: ContactQuery) -> list[SourceResult]:
        results: list[SourceResult] = []
        url = "https://www.egonzehnder.com/consultants"
            ...
            for profile in profiles[:10]:
                ...
                if name_match:
                    results.append(self._make_source_result(
                        ...
                        extracted_company="Egon Zehnder",
